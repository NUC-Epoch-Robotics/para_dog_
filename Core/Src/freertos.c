...
void RC_Ctrl(void const * argument)//现在用于雷达任务
{
  /* USER CODE BEGIN RC_Ctrl */
  extern vcp_message_t msg;
  radar_kalman_t radar_kf;
  with_kalman target;
  float t0, dt, t1;
  t1 = HAL_GetTick();
  radar_kalman_init(&radar_kf, 0.1f, 0.1f, 0.01f);  // 初始化dt设为0.01秒
  HAL_UART_Receive_IT(&huart2, (uint8_t *)&Rx_Temp, 1);
  /* Infinite loop */
  for(;;)
  {
    t0 = HAL_GetTick();
    dt = (t0 - t1) / 1000.0f;  // 转换为秒
    t1 = t0;
    if (dt > 0.0f && dt < 1.0f)  // 防止dt过大或负数
    {
      radar_kalman_predict(&radar_kf, dt);
      VCP_ReadTask();
      radar_kalman_update(&radar_kf, msg.xdata, msg.ydata);
      radar_kalman_get_state(&radar_kf, &target.x, &target.y, &target.vx, &target.vy);
    }
    HAL_UART_Transmit(&huart2, (uint8_t*)&msg, sizeof(vcp_message_t), HAL_MAX_DELAY);
